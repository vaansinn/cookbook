...
## Product context
Recipe Drawer teaches cooking through three difficulty tiers of the same dish (Basic/Intermediate/Advanced), Duolingo-style progress mechanics, shared household grocery lists, and computed nutrition. Full feature map and phased roadmap: see the blueprint artifact linked from `PIPELINE.md` and the `project-cookbook-site` memory. Basic tier is always free; paid-tier gating is undecided (P6).
